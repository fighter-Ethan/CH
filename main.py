...
@client.command()
async def amendment(ctx, *, number = "None"):
  if number == "None":
...
@client.command()
async def accept(ctx , user : discord.Member , role : discord.Role, crit = "None Provided"):
...
@client.command()
async def deny(ctx , user : discord.Member , role : discord.Role, crit = "None Provided"):
  await ctx.channel.purge(limit = 1)
  await ctx.send(f"{user.mention}")
...
async def verify(ctx, user : discord.Member):
  ...
  if verify.error:
    ...
    embed = discord.Embed(title = "**Welcome to Washington, D.C.!**", description = f"Please welcome {user.name}!", color = discord.Color.green())
    embed.set_footer(text = f"Verified by {ctx.author.name}")
    embed.set_image(url = "https://i.imgur.com/FTz2pMg.png")
    await welcomechat.send(embed=embed)
  
  if not verify.error:
  
    await user.send("Welcome to **Washington, D.C.!** To start out, please sign up for local congress or, if open, federal!")
